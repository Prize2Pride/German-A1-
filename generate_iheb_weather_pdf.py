...
Colors: Dark Red Bold (important), Dark Blue Bold (formal), Light Red (mistakes)
"""

from fpdf import FPDF
import os

class IhebWeatherPDF(FPDF):
    def __init__(self):
        super().__init__()
        ...
        self.DARK_BG = (30, 30, 40)        # Dark background
    ...
    def header(self):
        ...
        self.set_xy(10, 6)
        self.cell(0, 8, 'Prize2Pride German Course | Professor Roued', 0, 0, 'L')
        self.ln(18)
    ...
    def title_page(self):
        ...
        self.set_text_color(*self.WHITE)
        self.cell(0, 12, 'The Weather | الطقس', 0, 1, 'C')
        ...
        self.multi_cell(0, 8, 'A Winter Day in Tunisia with All Kinds of Weather\nيوم شتوي في تونس مع جميع أنواع الطقس', 0, 'C')
        ...
        self.set_text_color(*self.WHITE)
        self.cell(0, 8, 'Color Guide | دليل الألوان:', 0, 1, 'C')
        
        self.set_font('DejaVu', 'B', 10)
        self.set_text_color(*self.DARK_RED)
        self.cell(0, 7, 'Dark Red Bold = Important Expressions | تعبيرات مهمة', 0, 1, 'C')
        self.set_text_color(*self.DARK_BLUE)
        self.cell(0, 7, 'Dark Blue Bold = Grammar & Formal | قواعد ورسمي', 0, 1, 'C')
        self.set_font('DejaVu', '', 10)
        self.set_text_color(*self.LIGHT_RED)
        self.cell(0, 7, 'Light Red = Mistakes to Avoid | أخطاء يجب تجنبها', 0, 1, 'C')
...
def create_iheb_lesson():
    ...
    # ==================== VOCABULARY PAGE ====================
    pdf.add_page()
    pdf.section_title('Weather Vocabulary | مفردات الطقس', 'dark_red')
    
    vocab = [
        ['das Wetter', 'das VET-ter', 'the weather', 'الطقس'],
        ['die Sonne', 'dee ZON-ne', 'the sun', 'الشمس'],
        ['der Regen', 'dair RAY-gen', 'the rain', 'المطر'],
        ['der Schnee', 'dair SHNAY', 'the snow', 'الثلج'],
        ['der Wind', 'dair VINT', 'the wind', 'الرياح'],
        ['die Wolke', 'dee VOL-ke', 'the cloud', 'السحابة'],
        ['der Nebel', 'dair NAY-bel', 'the fog', 'الضباب'],
        ['das Gewitter', 'ge-VIT-ter', 'thunderstorm', 'العاصفة الرعدية'],
        ['der Blitz', 'dair BLITS', 'lightning', 'البرق'],
        ['der Donner', 'dair DON-ner', 'thunder', 'الرعد'],
        ['der Hagel', 'dair HAH-gel', 'hail', 'البرد'],
        ['der Sturm', 'dair SHTURM', 'storm', 'العاصفة'],
    ]
    pdf.vocab_table(vocab, ['German', 'Pronunciation', 'English', 'Arabic'])
    
    pdf.section_title('Weather Expressions | تعبيرات الطقس', 'dark_blue')
    ...
    # Mistakes to avoid
    pdf.section_title('Mistakes to Avoid | أخطاء يجب تجنبها', 'dark_red')
    pdf.mistake_text('WRONG: "Das Wetter regnet" - Weather does not rain!')
    pdf.mistake_text('CORRECT: "Es regnet" - It rains (impersonal)')
    pdf.arabic_text('خطأ: "الطقس يمطر" - الطقس لا يمطر! صحيح: "إنها تمطر" (صيغة غير شخصية)')
    pdf.ln(2)
    pdf.mistake_text('WRONG: "Ich bin kalt" - This means you are a cold person!')
    pdf.mistake_text('CORRECT: "Mir ist kalt" - I am cold (I feel cold)')
    pdf.arabic_text('خطأ: "أنا بارد" - هذا يعني أنك شخص بارد! صحيح: "أشعر بالبرد"')
    ...
    pdf.normal_text('(Dear ladies and gentlemen, I would like to report to you about my extraordinary day.)')
    ...
    # Formal vocabulary
    pdf.section_title('Formal Weather Vocabulary | مفردات الطقس الرسمية', 'dark_blue')
    formal_vocab = [
        ['die Witterung', 'weather conditions', 'الأحوال الجوية'],
        ['die Niederschlage', 'precipitation', 'الهطول'],
        ['die Luftfeuchtigkeit', 'humidity', 'الرطوبة'],
        ['die Temperatur', 'temperature', 'درجة الحرارة'],
        ['der Hochdruck', 'high pressure', 'الضغط العالي'],
        ['der Tiefdruck', 'low pressure', 'الضغط المنخفض'],
    ]
    pdf.vocab_table(formal_vocab, ['German', 'English', 'Arabic'])
    ...
    pdf.normal_text('At the end of the day, Mr. Iheb had experienced: sun, clouds, rain, wind, snow, and fog!')
    pdf.important_text('Am Ende des Tages hatte Herr Iheb erlebt: Sonne, Wolken, Regen, Wind, Schnee und Nebel!')
    pdf.arabic_text('في نهاية اليوم، عاش السيد إيهاب: الشمس، السحب، المطر، الرياح، الثلج والضباب!')
    pdf.ln(3)
    
    pdf.important_text('"Das war der verruckteste Tag meines Lebens! Ich liebe Tunesien!"')
    pdf.normal_text('("That was the craziest day of my life! I love Tunisia!")')
    pdf.arabic_text('"كان هذا أكثر يوم جنوناً في حياتي! أحب تونس!"')
    
    # ==================== FUNNY EXPRESSIONS ====================
    pdf.add_page()
    pdf.section_title('Funny German Weather Expressions | تعبيرات طقس مضحكة', 'dark_red')
    
    funny = [
        ['Hundewetter', 'Dog weather', 'Terrible weather', 'طقس فظيع'],
        ['Affenkalte', 'Monkey cold', 'Freezing cold', 'برد قارس'],
        ['Bullenhitze', 'Bull heat', 'Extremely hot', 'حر شديد'],
        ['Sauwetter', 'Pig weather', 'Awful weather', 'طقس سيء جداً'],
        ['Es regnet Bindfaden', 'Raining strings', 'Heavy rain', 'مطر غزير'],
    ]
    pdf.vocab_table(funny, ['German', 'Literal', 'Meaning', 'Arabic'])
    
    pdf.section_title('Weather Jokes | نكات الطقس', 'dark_red')
    
    pdf.important_text('Frage: Wie viele Jahreszeiten hat Tunesien?')
    pdf.important_text('Antwort: Alle vier - manchmal am selben Tag!')
    pdf.normal_text('(Question: How many seasons does Tunisia have? Answer: All four - sometimes on the same day!)')
    pdf.arabic_text('سؤال: كم فصلاً في تونس؟ جواب: الأربعة - أحياناً في نفس اليوم!')
    pdf.ln(5)
    
    pdf.important_text('Frage: Was sagt Herr Iheb jeden Morgen?')
    pdf.important_text('Antwort: "Ich brauche Sonnencreme, einen Regenschirm, eine Winterjacke und Sandalen!"')
    pdf.normal_text('(Question: What does Mr. Iheb say every morning? Answer: "I need sunscreen, an umbrella, a winter jacket, and sandals!")')
    pdf.arabic_text('سؤال: ماذا يقول السيد إيهاب كل صباح؟ جواب: "أحتاج واقي شمس، مظلة، معطف شتوي، وصندل!"')
    
    # ==================== SUMMARY PAGE ====================
    pdf.add_page()
    pdf.section_title('Lesson Summary | ملخص الدرس', 'dark_blue')
    ...
    pdf.section_title('Common Mistakes to Avoid | أخطاء شائعة يجب تجنبها', 'dark_red')
    pdf.mistake_text('1. "Das Wetter regnet" → WRONG! Use "Es regnet"')
    pdf.mistake_text('2. "Ich bin kalt" → WRONG! Use "Mir ist kalt"')
    pdf.mistake_text('3. "Es ist sonnig Wetter" → WRONG! Use "Es ist sonnig" or "Das Wetter ist sonnig"')
    ...
    # Save PDF
    output_path = '/home/ubuntu/German-A1-/lessons/07-weather/Prize2Pride_MrIheb_Weather_Lesson.pdf'
...
if __name__ == '__main__':
    create_iheb_lesson()
    print('Mr. Iheb Weather Lesson PDF generated successfully!')
